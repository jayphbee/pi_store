...
use {BlockBasedOptions, DBCompactionStyle, DBCompressionType, DBRecoveryMode,
	 Options, WriteOptions, TransactionDBOptions, TransactionOptions};
...
impl Default for TransactionDBOptions {
    fn default() -> TransactionDBOptions {
        unsafe {
            let opts = ffi::rocksdb_transactiondb_options_create();
            if opts.is_null() {
                panic!("Could not create RocksDB options");
            }
            TransactionDBOptions { inner: opts }
        }
    }
}
...
impl Default for TransactionOptions {
    fn default() -> TransactionOptions {
        unsafe {
            let opts = ffi::rocksdb_transaction_options_create();
            if opts.is_null() {
                panic!("Could not create RocksDB options");
            }
            TransactionOptions { inner: opts }
        }
    }
}
