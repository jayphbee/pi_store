/**
 * 用一个根日志文件不断追加写，记录所有子表的根块（子节点位置）。
 * 以根日志文件写入为准，表示索引记录落地。
 * 支持用指定的id创建新的子表，内存中会克隆sbtree，根文件中追加{块大小(4byte), {子表(4字节), 类型(1byte), 长度(3字节), [子节点位置(2字节或3字节)...]}, Crc32(4byte)}。
 * 根日志文件的格式为： [{块大小(4byte), [{子表id(4字节), 类型(1byte), 数组长度(3字节-可选), [{子节点位置(2字节或3字节)},...]=子节点位置数组, },...]-可选, Crc32(4byte)}, ...]，
