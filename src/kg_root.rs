...
use log::{read_3byte, Bin, Callback, SResult};
...
// 顺序加载根日志文件
fn load_root<P: AsRef<Path> + Send + 'static>(
    mut vec: Vec<(u32, P)>,
    file: P,
    tab: Tab,
    cb: Arc<Fn(SResult<Tab>)>,
) {
    match vec.pop() {
        Some((m, path)) => {
            AsyncFile::open(
                path,
                AsynFileOptions::ReadAppend(8),
                Box::new(move |f: IoResult<AsyncFile>| match f {
                    Ok(afile) => {
                        ...
                        afile.read(
                            0,
                            len as usize,
                            Box::new(move |f: AsyncFile, r: IoResult<Vec<u8>>| match r {
                                Ok(vec_u8) => {
...
fn next_open(tab: Tab, subtabs: Arc<SBMap>, cb: Arc<Fn(SResult<Tab>)>) {
    ...
    Record::init(
        tab.record.clone(),
        subtabs.clone(),
        Box::new(move |r: SResult<FnvHashMap<u32, Bin>>| {
            match r {
                Ok(rr) => {
